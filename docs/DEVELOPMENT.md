## Development of SampleCSharpNUnitSelenium
...
```bash
./SampleCSharpNUnitSelenium/script/dockercomposerun -d
```
...
```bash
./SampleCSharpNUnitSelenium/script/dockercomposerun -do
```
...
2. Run your development environment image in the docker compose
   ...
   ```bash
   BROWSERTESTS_IMAGE=browsertests-dev ./SampleCSharpNUnitSelenium/script/dockercomposerun -d
   ```
...
```bash
BROWSERTESTS_SRC=${PWD} BROWSERTESTS_IMAGE=browsertests-dev ./SampleCSharpNUnitSelenium/script/dockercomposerun -d
```
...
* To run the **tests**...
  ...
  * Sequentially
    ...
    ```bash
    ./SampleCSharpNUnitSelenium/script/run tests
    ```

  * In Parallel

    ```bash
    ./SampleCSharpNUnitSelenium/script/run -p tests
    ```

    :memo: Safari is native only and does not support running tests in parallel

* To run the **linting**...

  ```bash
  ./SampleCSharpNUnitSelenium/script/run lint
  ```

* To run the dependency **security scan**...

  ```bash
  ./SampleCSharpNUnitSelenium/script/run secscan
  ```
