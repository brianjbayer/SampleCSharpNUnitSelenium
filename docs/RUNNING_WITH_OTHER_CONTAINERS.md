...
2. From the project root directory, run the `dockercomposerun`
   ...
   ```bash
   BROWSER_TYPE=firefox SELENIUM_IMAGE=selenium/standalone-firefox ./SampleCSharpNUnitSelenium/script/dockercomposerun
   ```
...
2. From the project root directory, run the `dockercomposerun`
   ...
   ```bash
   BROWSER_TYPE=edge SELENIUM_IMAGE=selenium/standalone-edge ./SampleCSharpNUnitSelenium/script/dockercomposerun
   ```
