...
> `SampleCSharpNUnitSelenium/appsettings.json`
...
* `safari` - Apple Safari (local only, does not support parallel, requires Safari)
...
```bash
BROWSER_TYPE=firefox HEADLESS=true ./SampleCSharpNUnitSelenium/script/run tests
```
...
3. Run the tests specifying the remote Selenium container...
   ...
   ```bash
   REMOTE_URL='http://localhost:4444/wd/hub' BROWSER_TYPE=chrome ./SampleCSharpNUnitSelenium/script/run tests
   ```
